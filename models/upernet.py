...
class ResidualNet(nn.Module):
    def __init__(self, in_channel, block, layers) -> None:
        ...
        self.stem = nn.Sequential(
            ConvBnAct(
                in_channel, out_channel=64, kernel=3, stride=2, padding=1, dilation=1
            ),  # /2; c64
    ...
    def make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = ConvBnAct(
                self.inplanes, planes * block.expansion, 1, stride, 0, act=False
            )
...
class UPerNet(nn.Module):
    def __init__(self, in_channel=3, layers=[3, 4, 6, 3], num_class=20, fpn_dim=256):
        ...
        self.seg = nn.Sequential(
            ...
            nn.Conv2d(fpn_dim, num_class, 1, 1, 0, bias=True),
        )
    ...
    def forward(self, x):
        ...
        list_f.append(
            F.interpolate(
                fpn["fpn_layer2"], out_size, mode="bilinear", align_corners=False
            )
        )
        list_f.append(
            F.interpolate(
                fpn["fpn_layer3"], out_size, mode="bilinear", align_corners=False
            )
        )
        list_f.append(
            F.interpolate(
                fpn["fpn_layer4"], out_size, mode="bilinear", align_corners=False
            )
        )
        x = self.seg(self.fuse(torch.cat(list_f, dim=1)))
        pred = self.out(
            F.interpolate(x, seg_size, mode="bilinear", align_corners=False)
        )
...
if __name__ == "__main__":
    ...
    net = UPerNet(in_channel=3, layers=[3, 4, 6, 3], num_class=1)
    out = net(img)
    print(f"{out.shape}")
