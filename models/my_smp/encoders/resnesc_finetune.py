...
class EfficientAttention(nn.Module):
    def __init__(
        ...
        self.proj = (
            nn.Conv2d(dim, out_dim, 1, 1, 0, bias=qkv_bias)
            if final_proj
            else nn.Identity()
        )
...
class SegRSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.att_norm = nn.GroupNorm(
            out_channels // radix // radix, out_channels // radix
        )
        ...
        self.conv3 = nn.Conv2d(
            out_channels // radix, out_channels, kernel_size=3, stride=1, padding=1
        )
